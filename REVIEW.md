...
`SEC-1`, `BUG-4`, `REL-3`, `MISS-2`, `BUG-3`, `SEC-2`, `REL-1`, `MISS-1`, `MISS-3`, `MISS-4`

Deliberately not done: `CI-2`, `CI-3`, `MISS-5`. Each was either already covered by an earlier pass, or judged churn or too risky to make without a device or a measurement. The reasoning is in the commit that touched it.
...
Reversed (2026-09-23), in the TypeScript strictness row: `noUncheckedIndexedAccess` is on
as of 10aa4b1, because the shared conventions (`CONVENTIONS.md`) now require it in every
TypeScript project, and `test/conventions.mjs` fails when `tsconfig.json` does not resolve
it to true. It was not pure noise: resolving its errors found that a stored en-passant move
onto the first or last rank grew the board array. The row is kept as the record of the
original advice; the three flags it does recommend are still not set.
