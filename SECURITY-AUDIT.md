...
**Correction (follow-up review).** "Make ErrorBoundary.reset clear every key in STORAGE_KEYS plus the entitlement key" was taken too literally twice over. First, the escalation to clearing everything was keyed on a counter that nothing ever reset, so a second unrelated failure in one session — with the app rendering perfectly well in between — offered "Clear saved data" as the *only* button; that reasoning ("if it fails again the culprit is one of the others") holds only when there was no successful render in between, which nothing checked. The decisions now live in src/recovery.ts: the non-destructive recovery is always offered and always first, erasing is offered only when nothing has rendered since the last recovery (App.tsx's `SettleBeacon` is what says it has), and it asks before it does anything. Second, `clearAll` took the Pro entitlement with it while the copy listed only the other seven records. The entitlement is now in `KEPT_ON_CLEAR`: it is not the app's own state, `cleanEntitlements` means it cannot be the record that fails, and the bundled store restores nothing — so erasing it only ever lost someone their purchase. The confirmation copy names every record it drops and says the unlock is kept.

Status (2026-09-21): the `allowBackup` sentence above is stale as a vector description. Phase 2 (76ebe92) states `android.allowBackup: true` in app.json deliberately rather than leaving it to the plugin default, and `src/__tests__/appConfig.test.ts` pins it: the store is the player's own record — daily streak, ladder rank, puzzle progress and the Pro entitlement — with no server copy (PRIVACY.md: the app makes no network requests), so losing it on a device migration costs more than the privacy gain. What a crafted restore can plant is bounded where the fix to this finding put the bound: every record is read through `src/validate.ts` (`cleanStats`, `cleanDaily`, `cleanLadder`, `cleanPuzzleProgress`, `cleanEntitlements`, `checkSavedGame` with `MAX_EVENTS`, `cleanSettings` with own-property enum tables), so a planted record clamps to known values or is dropped with a note, and the ErrorBoundary's recovery can clear all of them but the entitlement.
...
**Fix.** In src/validate.ts: (1) cap the list — `if (g.events.length > MAX_EVENTS) return null;` with MAX_EVENTS around 2000, which is far above any real game [corrected below]; (2) replace isEvent with a per-type payload validator that rebuilds the event rather than passing the parsed object through — for 'move', require `from` and `to` to be integers with `from >= -1 && from <= 63` and `to >= 0 && to <= 63`, `piece`/`promotion`/`captured` to be members of the piece table (own-property lookup), and the flags to be booleans; for 'power', require `color` to be exactly 'w' or 'b', `level` to be an integer in [0, MAX_POWER] and `material`/`engine` to be finite; for 'accuse', require `caught` boolean and `by` undefined or 'ai'; (3) constrain `daily` to /^\d{4}-\d{2}-\d{2}$/ rather than any string, since it is the value that reaches the share sheet. Independently, make sanitizeEvents linear: fold once, applying events one at a time and remembering the last index that applied cleanly, instead of re-folding every prefix. Defence in depth: bound `m.to` in Position.makeMove (`if (m.to < 0 || m.to > 63) throw`), which costs nothing on the hot path that already only generates 0..63.

**Correction (follow-up review).** "MAX_EVENTS around 2000, which is far above any real game" was wrong. 402 ordinary pass-and-play games driven through the app's own event generation measured a median of 180 events, a 99th percentile of 1212 and a longest of 1676: 2000 was 1.19x the longest game observed, not far above it, and a game that crossed it had its record deleted with nothing shown to the player. The cap is now 8000 (~4.8x the longest measured), a record past it is clipped to the cap rather than deleted and `savedGameNote` says so where Resume was, and the replay is no longer quadratic — `replayablePrefix` in src/game/events.ts binary-searches the longest prefix that applies, which is exact because `fold` is a left fold, so the worst record the validator accepts replays in tens of milliseconds instead of the 9.3 s the scan cost at the same length. Both ends of the cap are pinned in src/__tests__/validate.test.ts: it may not fall below 4x the longest game measured, and it may not rise past what still replays inside a second.
